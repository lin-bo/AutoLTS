...
class OrdLabelMoCoHierarchy(nn.Module):

    def __init__(self, dim, queue_size=6400, momentum=0.999, temperature=0.07, n_chunk=4, alpha=1,
                 mlp=True, local=True, device='mps', simple_shuffle=False, weight_func='exp', inc_hl_dist=True):
        super(OrdLabelMoCoHierarchy, self).__init__()
        # initialize parameters
        self.queue_size = queue_size
        self.momentum = momentum
        self.temperature = temperature
    ...
    def forward(self, im_q, im_k, label):
        ...
        # set the label to the positive key indicator
        with torch.no_grad():
            targets = self.queue_lts.repeat(batch_size, 1)
            targets = torch.cat([torch.unsqueeze(label, 1), targets], dim=1)
        # dequeue and enqueue
        if not self.vali:
            self.dequeue_and_enqueue(k, label)
        return logits, targets
